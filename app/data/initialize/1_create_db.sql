...
CREATE TABLE IF NOT EXISTS wallets (
    ...
    -- Current Outstanding Debt / Cash
    available_credit REAL,
    -- Credit Limit minus all obligations
    credit_limit REAL,
    cycle_day INTEGER,
    due_day INTEGER,
    ...
    benefits TEXT,
    -- DEPRECATED: Use wallet_benefits table instead. JSON string: {"groceries": 4, "dining": 5, ...}
...
-- Normalized benefits table supporting both credit card cashback and debit card interest rates
-- Dictionary of Wallet Providers
CREATE TABLE IF NOT EXISTS providers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    wallet_type TEXT NOT NULL,
    -- 'credit', 'debit', 'digital_bank', 'ewallet', 'cash'
    logo_url TEXT,
    -- Optional: URL/path to logo icon
    is_active BOOLEAN DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
-- Seed Data for Providers
INSERT
    OR IGNORE INTO providers (name, wallet_type)
VALUES ('BPI', 'credit'),
    ('BDO', 'credit'),
    ('EastWest', 'credit'),
    ('BPI', 'debit'),
    ...
    ('GrabPay', 'ewallet'),
    ('Cash on Hand', 'cash');
CREATE TABLE IF NOT EXISTS wallet_benefits (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    wallet_id INTEGER NOT NULL,
    category_id INTEGER NOT NULL,
    benefit_type TEXT NOT NULL CHECK(benefit_type IN ('cashback', 'interest')),
    rate REAL NOT NULL,
    -- For credit: cashback % (e.g., 4.0 for 4%)
    -- For debit: interest rate % (e.g., 2.5 for 2.5% APY)
    is_active BOOLEAN DEFAULT 1,
    -- Allow temporary disabling of benefits
    effective_from TEXT,
    -- ISO8601 date when benefit starts
    effective_until TEXT,
    -- ISO8601 date when benefit ends (NULL = indefinite)
    notes TEXT,
    -- Optional: promo details, conditions
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (wallet_id) REFERENCES wallets(id) ON DELETE CASCADE,
    FOREIGN KEY (category_id) REFERENCES categories(id),
    UNIQUE(wallet_id, category_id, benefit_type) -- One rate per wallet-category-type combination
);
...
CREATE TABLE IF NOT EXISTS income_sources (
    ...
    frequency TEXT CHECK(
        frequency IN ('monthly', 'bi-weekly', 'weekly', 'sporadic')
    ),
    is_active BOOLEAN DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE IF NOT EXISTS budget_simulations (
    ...
    -- Optional: How does this affect your Japan Trip?
    recommendation TEXT,
    -- AI's Verdict: 'Go', 'No-Go', 'Wait'
    pilot_reasoning TEXT,
    -- The "Why" behind the AI advice
    impact_on_balance REAL,
    -- Projected balance after purchase
    confidence_score REAL,
    -- AI's certainty (0.0 to 1.0)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (wallet_id) REFERENCES wallets(id),
    FOREIGN KEY (category_id) REFERENCES categories(id),
...
CREATE TABLE IF NOT EXISTS strategic_recommendations (
    ...
    message TEXT NOT NULL,
    -- e.g., 'Switch to BPI for this Grocery run'
    urgency_level TEXT CHECK(
        urgency_level IN ('low', 'medium', 'high', 'critical')
    ),
    -- Notification Controls
    status TEXT DEFAULT 'pending' CHECK(
        status IN ('pending', 'dismissed', 'snoozed', 'acted_upon')
    ),
    reminder_frequency TEXT DEFAULT 'weekly' CHECK(
        reminder_frequency IN ('daily', 'weekly', 'bi-weekly')
    ),
    remind_at TIMESTAMP,
    -- The next scheduled time to notify the user
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
