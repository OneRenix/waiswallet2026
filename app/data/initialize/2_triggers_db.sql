...
CREATE TRIGGER IF NOT EXISTS trg_wallets_updated_at AFTER UPDATE ON wallets
BEGIN UPDATE wallets SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;

CREATE TRIGGER IF NOT EXISTS trg_transaction_headers_updated_at AFTER UPDATE ON transaction_headers
BEGIN UPDATE transaction_headers SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;

CREATE TRIGGER IF NOT EXISTS trg_transaction_details_updated_at AFTER UPDATE ON transaction_details
BEGIN UPDATE transaction_details SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;

CREATE TRIGGER IF NOT EXISTS trg_income_sources_updated_at AFTER UPDATE ON income_sources
BEGIN UPDATE income_sources SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;

CREATE TRIGGER IF NOT EXISTS trg_wallet_cashback_history_updated_at AFTER UPDATE ON wallet_cashback_history
BEGIN UPDATE wallet_cashback_history SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;

CREATE TRIGGER IF NOT EXISTS trg_recurring_expenses_updated_at AFTER UPDATE ON recurring_expenses
BEGIN UPDATE recurring_expenses SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;
...
BEGIN
    INSERT INTO wallet_ledger (
        wallet_id, entry_type, amount, previous_balance, new_balance, reason, created_at
    )
    VALUES (
        NEW.id,
...
-- Note: 'balance' tracks current debt, 'available_credit' tracks swiping power
...
BEGIN
    -- This trigger could update a 'recurring_id' column if we added one, 
...
AFTER INSERT ON transaction_details
FOR EACH ROW
WHEN NEW.cashback_earned > 0
BEGIN
    -- 1. Ensure the monthly history bucket exists (using the header's wallet)
    INSERT OR IGNORE INTO wallet_cashback_history (wallet_id, month_year, monthly_limit)
    SELECT h.wallet_id, strftime('%Y-%m', 'now'), w.monthly_cashback_limit
    ...
    WHERE wallet_id = (SELECT wallet_id FROM transaction_headers WHERE id = NEW.header_id)
    ...
    WHERE id = (SELECT wallet_id FROM transaction_headers WHERE id = NEW.header_id);
END;